...
-}
subList :: Int -> Int -> [a] -> [a]
subList a b ls = if b - a > 0
  then drop a (take (b + 1) ls)
  else []
...
smartReplicate l = concat (map partialReplicate l)
                where partialReplicate n = replicate n n
...
-}
rotate :: Int -> [a] -> [a]
rotate n ls = subList n (length ls + n - 1) (cycle ls)
...
rewind ls = go ls []
  where
    go :: [a] -> [a] -> [a]
    go [] new_l = new_l
    go l new_l = new_l ++ [last l] ++ go (init l) new_l
